...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        // FIXED: pre-warm all scenes before the timer starts
        // Original: start() called lazily on scene switch → multi-ms spike mid-frame
        // Fixed:    all heavy init done here, off the hot path
        let pw = "prewarm".withCString { n in
            "AppDelegate.swift".withCString { f in tracy_zone_begin(n, f, 29) }
        }
        imageScene.start()
        dataScene.start()
        tracy_zone_end(pw)
        "All scenes pre-warmed".withCString { tracy_message_l(2, 0, $0) }
    ...
    func gameTick() {
        ...
        case 3..<6:
            if window.contentView !== imageScene { window.contentView = imageScene }
            imageScene.update()
            imageScene.needsDisplay = true
        default:
            if window.contentView !== dataScene { window.contentView = dataScene }
            dataScene.update()
